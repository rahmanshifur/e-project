...
function Update({ editItem, editHandler }) {
    ...
    const update = useStoreActions(action => action.user.update)
    ...
    useEffect(() => {
        ...
        console.log('editItem', editItem.status)

        let country = {}
        let state = {}
        let city = {}
        let street = streetData.filter(item => item.id === editItem.streetId)[0]
        if (street) {
            city = cityData.filter(city => city.id === street.cityId)[0]
        }
        if (city) {
            state = stateData.filter(state => state.id === city.stateId)[0]
        }
        if (state) {
            country = countData.filter(count => count.id === state.countryId)[0]
        }
        setCountry(country.id || 0)
        setState(state.id || 0)
        setCity(city.id || 0)
        setStreetId(street.id || 0)

    }, [editItem, streetData, stateData, cityData, countData])
    ...
    const submitHandler = (e) => {
        ...
        update({ id: editItem.id, ...obj, successHandler })
    }
    ...
    return (
        <Form onSubmit={submitHandler} className="p-3 mt-3 bg-warning">
            ...
            <Button className='mt-3' size='lg' color='primary' type='submit'>Update</Button>
        </Form>
    )
}
